...
function Button({
  ...
  // If asChild is true and we have icons or loading, we can't use Slot
  // because we need to control the content structure
  if (asChild && (loading || iconLeft || iconRight)) {
    console.warn(
      "Button: Cannot use icons or loading state with asChild prop. Rendering as button instead.",
    );
  }

  const shouldUseSlot = asChild && !loading && !iconLeft && !iconRight;
  const Component = shouldUseSlot ? Slot : "button";
